...
- **Session–environment cardinality.** A session is NOT 1:1 with an environment. Each record carries its own `environment_id`, so a session is 1:N to environments. Today, with local-only execution, all records in a session share one environment, but distributed execution (cluster, cloud) will produce records from different nodes. The session view currently shows the environment from the first record as a convenience summary, but this is a display shortcut — the authoritative environment for each observation is on the record itself. This also raises the question of **environment equivalence**: when aggregating or comparing observations, what delta between environments is acceptable to consider them "comparable"? Exact hash match is too strict (kernel patch versions, CPU frequency stepping), but too-loose matching conflates meaningfully different hardware. This needs a similarity/tolerance model before cross-environment aggregation is useful.
